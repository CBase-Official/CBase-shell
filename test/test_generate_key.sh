...
RESULT=$(./bin/cbase generate-key generate-key-test --networkId $NODE_ENV)
echo $RESULT
...
RESULT2=$(./bin/cbase generate-key generate-key-test --networkId $NODE_ENV)
echo $RESULT2

EXPECTED2=".*Account has existing key pair with ed25519:.+ public key.*"
if [[ ! "$RESULT2" =~ $EXPECTED2 ]]; then
    echo FAILURE Unexpected output from cbase generate-key when key already exists
    exit 1
fi